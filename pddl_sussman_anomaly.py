...
class GeneratePDDL_Stationary :
    ...
    def generateInitString(self) :
        ...
        '''
        initString =  "(on_table B T)(on_table A T)(on_block C A)(clear_blocktop B)(clear_blocktop C)"       
        return initString
    ...
    def generateGoalString(self) :
        ...
        '''
        return "(and (on_block A B) (on_block B C) (on_table C T) (clear_blocktop A))"
...
def initializeSystem(env):
    gen = GeneratePDDL_Stationary(env, len(env.lanes), width=env.width, file_name='sussmananomaly')
    return gen


def generateDomainPDDLFile(gen):
    gen.addDomainHeader("Sussman_Anomaly")
    gen.addTypes(types = {"block" : None, "table" : None})
    gen.addPredicate(name="on_block", parameters=(("K1" , "block"), ("K2", "block")))
    gen.addPredicate(name="on_table", parameters=(("K" , "block"), ("E", "table")))
    gen.addPredicate(name="clear_blocktop", parameters=[("K" , "block")], isLastPredicate=True)


    gen.addAction(name="STACKONBLOCK",
                  parameters=(("K1" "block"), ("K2","block")),
                  precondition_string="(and (clear_blocktop ?K2) (clear_blocktop ?K1) (on_block ?K1 ?K2))",
                  effect_string="(not (clear_blocktop ?K2))")
    gen.addAction(name="PLACEONTABLE",
                  parameters=(("K" "block"), ("E","table")),
                  precondition_string="(and (clear_blocktop ?K) (not (on_table ?K ?E)))",
                  effect_string="(on_table ?K ?E)")
    gen.addAction(name="REMOVEFROMTABLE",
                  parameters=(("K" "block"), ("E","table")),
                  precondition_string="(and (clear_blocktop ?K) (on_table ?K ?E))",
                  effect_string="(not (on_table ?K ?E))")

    gen.generateDomainPDDL()
    pass

def generateProblemPDDLFile(gen):
    gen.addProblemHeader("blockstack", "block_world")
    gen.addObjects("block", ["A","B","C"],isLastObject=True)
